...
walk(path_dat, function(x){
  ...
  island_code <- sfc_factor |> filter(island_name == target_island) |> pull(island_code)
  ...
  sf_ph2 <- sf_point_phase2 |> filter(lu_access2 == "Forest-access") 
  ...
  sf_tof <- sf_point_phase2 |> filter(lu_access2 == "TOF-access")
  ...
  count_plot_phase <- ceiling(nrow(sf_ph2)/5)
  count_phase <- rep(1:5, count_plot_phase)
  count_phase <- count_phase[1:nrow(sf_ph2)]
  ...
  sf_for <- sf_ph2 |>
    arrange(dist) |>
    mutate(
      plot_num = 1:nrow(sf_ph2),
      plot_id_ph2 = case_when(
        plot_num < 10 ~ paste0(island_code, "F00", plot_num),
        plot_num < 100 ~ paste0(island_code, "F0", plot_num),
        TRUE ~ paste0(island_code, "F", plot_num)
        ),
      plot_phase = count_phase
      )
  
  sf_for$plot_id_ph2
  print(table(sf_for$plot_phase))
  
  gr <- ggplot() +
    geom_sf(data = sf_strata, fill = "grey95", color = NA) +
    geom_path(data = sf_point, aes(x = x, y = y), color = "grey80") +
    geom_sf(data = sf_for, aes(color = plot_num)) +
  ...
  gr <- ggplot() +
    geom_sf(data = sf_strata, fill = "grey95", color = NA) +
    geom_path(data = sf_point, aes(x = x, y = y), color = "grey80") +
    geom_sf(data = sf_for, aes(color = factor(plot_phase))) +
    scale_color_viridis_d(direction = -1) +
    theme_void() +
    theme(legend.position = "none") +
    labs(color = "") +
    facet_wrap(~plot_phase)
  print(gr)
  ggsave(gr, filename = paste0(path_res, "/", target_island, "_accessible_forest_5phases.jpeg"), width = 12, height = 9, dpi = 300, units = "cm")
  
  sf_for |>
    as_tibble() |>
    select(-email) |>
    write_csv(file.path(path_res, paste0(target_island, "_PH2_samples_forest.csv")))
...
tt <- ceo2 |> filter(lu_access2 == "Forest-access")
table(tt$lu_sub, tt$pl_island)

file_path <- list.files(path_res, pattern = "forest.csv", full.names = T)
sf_for_all <- map(file_path, read_csv) |> list_rbind()

table(sf_for_all$island, sf_for_all$plot_phase)


# sf_sfc <- st_read("data-raw/Ph1-centroid-withSFC/Kalimantan-ph1-withSFC.gpkg") |>
#   mutate(
#     plotid = PLOTID, 
#     pl_fra_hexid = as.character(FRA_HEXID)
#   )
...
# n_plot <- 413
# 
# ran_start_range <- 1:ceiling(n_pool/n_plot * 2.5)
# 
# ## !!! Temporary setting a seed to make results stable
# set.seed(10)
# start_point <- sample(ran_start_range, 1)
# 
# ## Get sampling proportion of available plots
# sampling_proportion <- n_plot / n_pool * 2.5
# sampling_proportion
# 
# ## Initialize for loop
# pool_seqnum <- sf_point$FRA_HEXID
...
# table(sf_point_phase2$lu_access)
# 
# sf_ph2 <- sf_point_phase2 |> filter(lu_access == "Forest-access") 
# table(sf_ph2$lu_sub, sf_ph2$lu_cat)
# 
# gr <- ggplot() +
#   geom_sf(data = sf_ph2, aes(color = lu_sub)) +
#   geom_path(data = sf_point, aes(x = x, y = y)) +
